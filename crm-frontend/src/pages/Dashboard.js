...
const [saleCount, setSaleCount] = useState(0);
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      console.log("Leads", l.data);
      console.log("Tasks", t.data);
      console.log("Sales", s.data);
...
setCustomerCount(c.data.totalElements);
setLeadCount(l.data.totalElements);
setTaskCount(t.data.totalElements);
setSaleCount(s.data.totalElements);
      } catch (err) {
  ...
  <div>
    ...
    <p>
      ...
      <div className="stat-grid">
        <div className="stat-card" style={{ '--rail-color': 'var(--primary)' }}>
          <div className="stat-label">Customers</div>
          <div className="stat-value">{loading ? '—' : customerCount}</div>
        </div>
        <div className="stat-card" style={{ '--rail-color': 'var(--warning)' }}>
          <div className="stat-label">Active Leads</div>
          <div className="stat-value">{loading ? '—' : activeLeads}</div>
        </div>
        <div className="stat-card" style={{ '--rail-color': 'var(--accent)' }}>
          <div className="stat-label">Open Tasks</div>
          <div className="stat-value">{loading ? '—' : openTasks}</div>
        </div>
        <div className="stat-card" style={{ '--rail-color': 'var(--success)' }}>
          <div className="stat-label">Revenue Won</div>
          <div className="stat-value">${loading ? '—' : wonRevenue.toLocaleString()}</div>
        </div>
      </div>
